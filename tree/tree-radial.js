...
d3.json("../datatest.json", function(json) {
		...
		if(d.depth == 0) return "rgba(128,0,128,0.5)";
		...
		node.append("image")
			...
			.attr("x", function(d){return (-1)*calculateRadius(d);})
			.attr("y", function(d){return (-1)*calculateRadius(d);})
			.attr("width", function(d){return 2*calculateRadius(d);})
			.attr("height", function(d){return 2*calculateRadius(d);})
